    ...
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_films_afficher_data = """SELECT * FROM t_client """
                # Prenom, Nom, Mail, Telephone, Disponibilite,GROUP_CONCAT(intitule_genre) as GenresFilms FROM t_materiel
                                                            #RIGHT JOIN t_client ON t_client.id_client = t_genre_film.fk_materiel
                                                            #LEFT JOIN t_client ON t_client.id_client = t_genre_film.fk_genre
                                                            #GROUP BY id_film"""
                if id_film_sel == 0:
...
"""
    ...
    1) Tous les genres contenus dans la "t_client".
...
def edit_genre_film_selected():
    if request.method == "GET":
        try:
            ...
            # Extrait les valeurs contenues dans la table "t_client", colonne "intitule_genre"
            # Le composant javascript "tagify" pour afficher les tags n'a pas besoin de l'id_client
            lst_data_genres_films_non_attribues = [item['description'] for item in data_genres_films_non_attribues]
...
def update_genre_film_selected():
    if request.method == "POST":
        try:
            ...
            # OM 2021.05.02 Une liste de "id_client" qui doivent être effacés de la table intermédiaire "t_genre_film".
            ...
            # Une liste de "id_client" qui doivent être ajoutés à la "t_genre_film"
            ...
            # "fk_film"/"id_film" et "fk_genre"/"id_client" dans la "t_genre_film"
            strsql_insert_genre_film = """INSERT INTO t_installation (id_installation, fk_materiel, fk_client)
                                                    VALUES (NULL, %(value_fk_materiel)s, %(value_fk_client)s)"""

            # SQL pour effacer une (des) association(s) existantes entre "id_film" et "id_client" dans la "t_genre_film"
            strsql_delete_genre_film = """DELETE FROM t_installation WHERE fk_client = %(value_fk_genre)s AND fk_film = %(value_fk_film)s"""
...
def genres_films_afficher_data(valeur_id_film_selected_dict):
    ...
    try:
        ...
        strsql_film_selected = """SELECT id_film, nom_film, duree_film, description_film, cover_link_film, date_sortie_film, GROUP_CONCAT(id_client) as GenresFilms FROM t_genre_film
                                        INNER JOIN t_film ON t_film.id_film = t_genre_film.fk_film
                                        INNER JOIN t_client ON t_client.id_client = t_genre_film.fk_genre
                                        WHERE id_film = %(value_id_film_selected)s"""

        strsql_genres_films_non_attribues = """SELECT id_client, intitule_genre FROM t_client WHERE id_client not in(SELECT id_client as idGenresFilms FROM t_genre_film
                                                    INNER JOIN t_film ON t_film.id_film = t_genre_film.fk_film
                                                    INNER JOIN t_client ON t_client.id_client = t_genre_film.fk_genre
                                                    WHERE id_film = %(value_id_film_selected)s)"""

        strsql_genres_films_attribues = """SELECT id_film, id_client, intitule_genre FROM t_genre_film
                                            INNER JOIN t_film ON t_film.id_film = t_genre_film.fk_film
                                            INNER JOIN t_client ON t_client.id_client = t_genre_film.fk_genre
                                            WHERE id_film = %(value_id_film_selected)s"""
